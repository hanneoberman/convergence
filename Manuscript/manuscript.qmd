...
$$
where $\epsilon \sim \mathcal{N}(0, 100)$. This results in a dataset $y$, where $y_{\mathrm mis} = \emptyset$. In other words, $y$ contains the complete set of observations for all units $i$ on all variables $j$ (where $j =  Y, X_1, X_2, X_3$). From the complete dataset we obtain the true values of the scientific estimands $Q$. 
...
In each simulation repetition, we *ampute* the complete dataset conform the missingness conditions. The proportion of incomplete cases $p_{\mathrm inc}$ varies between 5% and 95%, in incremental steps ($p_{\mathrm inc}$ = .05, .25, .50, .75, .95). We consider all possible univariate and multivariate patterns of missingness, conform a 'missing completely at random' missingness mechanism [MCAR; @rubin87]. I.e., the probability to be missing is the same for all $N \times k$ cells in $y$, conditional on $p_{\mathrm inc}$. We use the 'mice' package [function `mice::ampute()`; @mice] to obtain an incomplete dataset {$y_{\mathrm obs}, y_{\mathrm mis}$} for each of the five missingness conditions. 
